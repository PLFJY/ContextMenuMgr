...
ShellVerb visibility mutation is a physical-path transaction. ContextMenuMgrPlus uses only `ProgrammaticAccessOnly` as its managed hide marker, persists the exact pre-mutation value/kind plus a physical-generation fingerprint, freshly reopens the target for verification, then commits desired/observed state. `CommandFlags` is preserved as `EXPCMDFLAGS`; enable restores owned provenance instead of deleting third-party visibility metadata. Failures after a write use compare-before-restore rollback and report a verified rollback separately from an external-change conflict. For a protected machine `HKLM\SOFTWARE\Classes` verb, the existing narrow `ProtectedRegistryMutation` fallback remains unchanged: only ACL access denied can invoke it, it restores the original owner/DACL, it never applies to `HKEY_USERS\<SID>`, and it never bypasses Registry Write Protection.

For File-category verbs, `open` is protected when the parent `shell` selects it or the frontend user's effective extension association resolves to its ProgID. Generic toggle, command-edit and attribute-edit operations reject that target before mutation. `print` and `printto` remain manageable and their per-item transaction never rewrites sibling `open`, parent defaults, commands, `UserChoice`, or a separate `SystemFileAssociations` source. HKU/HKLM duplicates are verified against the selected physical source rather than forced into identical states. Win11 packaged COM blocking remains a separate CLSID blocked-list domain.

Classic cascading Shell Verbs expose `CanManageSubMenuItems` in the lightweight snapshot but enumerate children only through `GetShellSubMenuItems`. `SubCommands` children are enabled/disabled by changing the current parent's reference list, never by hiding the shared CommandStore command. Removed references are persisted with their parent identity and original order, then restored against the latest live value so installer additions are preserved. `ExtendedSubCommandsKey` and parent-local `shell` children use the physical ShellVerb control domain; an Extended target is marked shared because it can be used by more than one parent. Known dynamic Shell Extensions such as 7-Zip and Bandizip are deliberately different: their individual menu options are exposed only when their CLSID has a curated `DetailedEditDic.xml` entry, whose rules target the vendor's documented configuration values rather than arbitrary COM menu children.
...
`ShellExecute` 只在确实需要 ShellExecute 专属行为时才保留旧的 `mshta vbscript:createobject("shell.application").shellexecute(...)` 包装，例如显式非空 `Directory` 或未知 ShellExecute 属性。空 `<ShellExecute/>`、`Verb="open"`，以及只有 `WindowStyle` 的增强菜单项会直接写成 `<FileName> <Arguments>`，避免多层 XML / C# / 注册表 / mshta / VBScript 转义导致简单命令失效。`Verb="runas"` / 管理员增强菜单项默认编译为独立的 `powershell.exe ... Start-Process -Verb RunAs -Wait -PassThru ...` 命令；复杂管理员动作可在字典中使用 `PowerShellScript`，由后端生成提升后的 PowerShell command block，并把提升子进程的 exit code 传回调用方。`PowerShellScript` 默认提升；仅当脚本必须留在调用它的 Explorer 用户上下文（例如停止后重启该用户的 Explorer）时，才可显式使用 `Elevate="false"`。缓存刷新会优先让 Winlogon 的 `AutoRestartShell` 自动恢复 Explorer，仅在该设置明确为 `0` 时才手动启动，避免双重启动和额外的文件管理器窗口。脚本中的环境变量保留到 Explorer 实际调用命令时才解析，不能在服务注册命令时展开为 SYSTEM profile 路径。启用后的命令仍然只依赖系统工具、注册表命令值和必要的持久生成脚本，不依赖 ContextMenuMgr 前端、后端、TrayHost 或 pipe。
...
`CategoryPageView` 中的传统菜单项可以携带稳定 item id 跳转到 `ApplicationGroupsPage`。跳转请求复用 `GlobalSearchNavigationFilterService` 的 pending request 机制；按应用管理页面按 item id 找到目标传统菜单项，进入导航精确筛选模式，只渲染目标项所属的一个分组，并且该分组内只包含目标项本身。搜索框显示目标项的可读名称；用户手动修改搜索框或点击清除筛选后，页面退出精确筛选并恢复普通分组搜索。该定位过程只使用前端已加载数据，不触发新的后端、注册表、文件系统查询或导航后滚动定位。
